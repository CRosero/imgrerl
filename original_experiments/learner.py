...
from pomdp_baselines.policies.models import AGENT_CLASSES, AGENT_ARCHS
from pomdp_baselines.torchkit.networks import ImageEncoder

# Markov policy
from pomdp_baselines.buffers.simple_replay_buffer import SimpleReplayBuffer
...
from pomdp_baselines.torchkit import pytorch_utils as ptu
from pomdp_baselines.utils import evaluation as utl_eval
from pomdp_baselines.utils import logger
    ...
    def collect_rollouts(self, num_rollouts, random_actions=False):
        ...
        for idx in range(num_rollouts):
            ...
            if self.env_type == "meta" and self.train_env.n_tasks is not None:
                ...
                obs, info = self.train_env.reset(task=task)  # reset task
                obs = ptu.from_numpy(obs)
            else:
                obs, info = self.train_env.reset()  # reset task
    ...
    def evaluate(self, tasks, deterministic=True):
        ...
        for task_idx, task in enumerate(tasks):
            ...
            if self.env_type == "meta" and self.eval_env.n_tasks is not None:
                obs, info = self.eval_env.reset(task=task)  # reset task
                obs = ptu.from_numpy(obs)
                observations[task_idx, step, :] = ptu.get_numpy(obs[:obs_size])
            else:
                obs, info = self.eval_env.reset()  # reset task
                obs = ptu.from_numpy(obs)
